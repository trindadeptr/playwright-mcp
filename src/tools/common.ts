...
export const goBack: ToolFactory = (snapshot) => ({
  ...
  handle: async (context) => {
    return await context.runAndWait(
      async (page) => {
        await page.goBack();
      },
      {
...
export const wait: Tool = {
  ...
  handle: async (context, params) => {
    ...
    const maxWaitTimeInSeconds = 30 * 60;
    await new Promise((f) =>
      setTimeout(
        f,
        Math.min(1000 * maxWaitTimeInSeconds, validatedParams.time * 1000)
      )
    );
    return {
      content: [
...
export const pressKey: (captureSnapshot: boolean) => Tool = (
  captureSnapshot
) => ({
  schema: {
...
export const pdf: Tool = {
  ...
  handle: async (context) => {
    ...
    const fileName =
      path.join(
        os.tmpdir(),
        sanitizeForFilePath(`page-${new Date().toISOString()}`)
      ) + ".pdf";
